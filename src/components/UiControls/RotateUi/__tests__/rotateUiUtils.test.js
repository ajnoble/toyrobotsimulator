...
const createDirectionObject = direction => ({
  direction: direction
});
describe("rotateUiUtils: getNewDirection()", () => {
  const north = createDirectionObject("north");
  const east = createDirectionObject("east");
  const south = createDirectionObject("south");
  const west = createDirectionObject("west");
